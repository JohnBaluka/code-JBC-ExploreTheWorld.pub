...
namespace JBC.ExploreTheWorld.DL.MsOfficeApi.NetOffice_Impl.JsonWriters
{
    ...
    // Word document (via the strongly-typed NetOffice object model) and writes it
    ...
    public static class MsWordJsonWriter
    {
        ...
        private static WD.Shape BuildShape(Shape shape)
        {
            var entity = new WD.Shape
            {
                ...
                HasTextFrame = Bool(() => ((dynamic)shape.UnderlyingObject).HasTextFrame),
            };
        ...
        private static WD.Paragraph BuildParagraph(Paragraph paragraph)
        {
            return new WD.Paragraph
            {
                ...
                Style = Str(() => ((Style)paragraph.Style).NameLocal),
