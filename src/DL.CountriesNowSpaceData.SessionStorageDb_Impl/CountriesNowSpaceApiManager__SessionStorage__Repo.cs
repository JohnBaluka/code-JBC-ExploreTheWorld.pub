...
{
    // Browser sessionStorage implementation of CountriesNowSpaceApiManager__Repo__Interface.
    // Entities are serialized as JSON lists keyed by constant storage keys.
    // Data persists for the browser tab session only; cleared when the tab is closed.
    // All operations are async; read methods return empty when the key has not been written yet.
    public class CountriesNowSpaceApiManager__SessionStorage__Repo
        : CountriesNowSpaceApiManager__Repo__Interface
    {
        ...
        public CountriesNowSpaceApiManager__SessionStorage__Repo(ISessionStorageService storage)
        {
            _storage = storage;
        }
