...
namespace JBC.ExploreTheWorld.AL.BlazorLib.Server
{
    // Classic Program.Main (not top-level statements): Radzen Blazor Studio's design-time
    // compilation rejects top-level-statement locals (CS8801), so the host uses an explicit
    // entry point (mirrors the code-JBC-DoMoreWithSlides AL.BlazorLib.Server._radzen reference).
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            // The WASM client (AL.BlazorLib._radzen) registers BrowserNewWindow_AppService for interactive use.
            ...
            // Explicit collection initializers (not C# 12 [ ] collection expressions): Radzen Studio's
            ...
            {
                ProviderName       = builder.Configuration["DbProvider"] ?? DbProviderNames.InMemoryDb,
                AvailableProviders = new List<string>
                {
                    DbProviderNames.InMemoryDb,
                    DbProviderNames.AccessDb,
                    DbProviderNames.SqliteDb,
                    DbProviderNames.SqlServerDb,
                },
            };
