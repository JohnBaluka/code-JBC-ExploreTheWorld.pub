...
[COMAddin("JBC ETW PowerPoint Watcher", "ExploreTheWorld PowerPoint event watcher", 3)]
[ProgId("JBC.ExploreTheWorld.AL.MsOfficePowerPointVstoAddIn._netF.Addin")]
[Guid("A6F7A8B9-D0E1-4C34-F012-345678901234")]
[CustomUI("RibbonUI.xml", true)]
// NOTE: no [CustomPane]. PowerPoint is SDI — every presentation has its own document window and a
// Custom Task Pane belongs to exactly one window. We create panes per active window on demand
// (keyed by window HWND) instead of the single [CustomPane] pane. See "Per-window CTPs" below.
public class Addin : COMAddin
{
    // ------------------------------------------------------------------ //
    //  Per-window Custom Task Panes (PowerPoint SDI)
    // ------------------------------------------------------------------ //
    private const string WatcherPaneProgId   = "JBC.ExploreTheWorld.AL.WinFormsLib.MsPowerPoint_Watcher_UserControl";
    private const string CountriesPaneProgId = "JBC.ExploreTheWorld.AL.MsOfficePowerPointVstoAddIn.CountriesNow_TaskPane_UserControl";
    private const string WatcherPaneTitle    = "ETW MsOfficePowerPointVstoAddIn._netF — Watcher";
    private const string CountriesPaneTitle  = "ETW MsOfficePowerPointVstoAddIn._netF — Countries API";
    ...
    // When this add-in is COM-activated into POWERPNT.EXE, dependencies are resolved against the
    // host's default AppDomain — whose probe path is PowerPoint's install folder and the GAC, NOT
    // this add-in's bin directory. Some dependencies (e.g. Microsoft.Bcl.AsyncInterfaces, pulled in
    // via System.Text.Json) are also deployed at a higher version than callers reference, and a
    ...
    static Addin()
    {
        ...
        try
        {
            ...
            NetOffice.DebugConsole.Default.FileName = Path.Combine(Path.GetTempPath(),
                "JBC.ExploreTheWorld.AL.MsOfficePowerPointVstoAddIn._netF.netoffice.log");
        }
        catch { }
    ...
    // There are no [CustomPane]s to create — panes are made per presentation window on demand.
    ...
    private static int SafeHwnd(PowerPoint.DocumentWindow window)
    {
        try { return window?.HWND ?? 0; } catch { return 0; }
    }

    private PowerPoint.DocumentWindow ActiveWindow()
    {
        try { return Application?.ActiveWindow; } catch { return null; }
    }

    // Get (or lazily create) the pane of the given kind for a specific presentation window.
    private Office.CustomTaskPane GetOrCreatePane(
        Dictionary<int, Office.CustomTaskPane> map, string progId, string title, int width, PowerPoint.DocumentWindow window)
    {
        ...
        try
        {
            ...
            {
                try { if (pane.ContentControl is MsPowerPoint_Watcher_UserControl uc) uc.InjectApplication(Application); }
    ...
    private void OnPaneVisibleStateChanged(Office._CustomTaskPane customTaskPaneInst)
    {
        try
        {
            ...
            try { RibbonUI.InvalidateControl("etw_PowerPointTaskPane"); } catch { }
        ...
        try
        {
            var thread = new System.Threading.Thread(() =>
            {
                try
                {
                    ...
                    form.EnableAddinExportMode("PowerPoint");
    ...
    public void OnOpenWatcher(Office.IRibbonControl control)
    {
        try
        {
            ...
            var uc = new MsPowerPoint_Watcher_UserControl { Dock = System.Windows.Forms.DockStyle.Fill };
    ...
    public void OnSaveAsJson(Office.IRibbonControl control)
    {
        try
        {
            var prs = Application?.ActivePresentation;
            if (prs == null) return;
            var thread = new System.Threading.Thread(() =>
            {
                try
    ...
    {
        try { Application?.Run("MSO_MsPowerPoint.WriteActivePresentation"); }
    ...
    private static void LogError(string context, Exception ex)
    {
        try
        {
            string path = Path.Combine(Path.GetTempPath(), "JBC.ExploreTheWorld.AL.MsOfficePowerPointVstoAddIn._netF.log");
