...
{
    [ComVisible(true)]
    [Guid("A1B2C3D4-E5F6-7890-ABCD-EF0123456702")]
    /// <summary>
    /// Hosts <see cref="MsWord_Watcher_Form"/> as a reusable UserControl
    /// so it can be embedded in VSTO Task Panes via ICTPFactory.CreateCTP.
    /// Implements <see cref="ITaskPane"/> so NetOffice injects the running Word
    ...
    /// </summary>
    public class MsWord_Watcher_UserControl : UserControl, ITaskPane, IObjectSafety
#if !NETFRAMEWORK
...
#endif
    {
        ...
        private readonly MsWord_Watcher_Form _form;

        public MsWord_Watcher_UserControl()
        {
            _form = new MsWord_Watcher_Form();
        ...
        /// <summary>
        /// Injects a running Word.Application into the embedded watcher form.
        ...
        public void OnConnection(NetOffice.WordApi.Application application, Office._CustomTaskPane parentPane, object[] customArguments)
        {
