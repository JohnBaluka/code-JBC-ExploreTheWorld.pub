...
{
    /// <summary>
    /// Late-bound (dynamic) COM <c>AddPicture</c> calls require a file on disk. This helper returns
    /// the local cache path carried on the row when available, and otherwise materializes the PNG
    /// bytes to a per-user temp folder (<c>%TEMP%\ETW_FlagImages</c>) that acts as a secondary cache
    /// for COM-based exports — mirroring the Interop/NetOffice flag helpers.
    /// </summary>
    internal static class DynamicFlagImage_Helper
    {
