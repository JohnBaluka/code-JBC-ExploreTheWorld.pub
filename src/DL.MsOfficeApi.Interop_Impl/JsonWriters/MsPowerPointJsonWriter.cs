...
// These shape-formatting type names exist in both Microsoft.Office.Core and
// Microsoft.Office.Interop.PowerPoint; the writer always means the PowerPoint ones.
using Shape = Microsoft.Office.Interop.PowerPoint.Shape;
using FillFormat = Microsoft.Office.Interop.PowerPoint.FillFormat;
using LineFormat = Microsoft.Office.Interop.PowerPoint.LineFormat;
...
namespace JBC.ExploreTheWorld.DL.MsOfficeApi.Interop_Impl.JsonWriters
{
    ...
    // PowerPoint presentation (via the strongly-typed Microsoft.Office.Interop object model) and writes
    ...
    public static class MsPowerPointJsonWriter
    {
        ...
        private static PP.Presentation BuildPresentation(Presentation p, string outputFilePath, MsOfficeJsonWriterOptions options, Action<string>? log)
        {
            ...
            prs.AutoSaveOn = Bool(() => ((dynamic)p).AutoSaveOn); // absent from the Office15 PIA — read late-bound
            ...
            // Microsoft.Office.Interop surfaces these MsoTriState properties as bool; the canonical
            ...
            prs.ReadOnlyRecommended = Bool(() => ((dynamic)p).ReadOnlyRecommended); // absent from the Office15 PIA — read late-bound
        ...
        private static PP.Hyperlink BuildHyperlink(Hyperlink hyperlink)
        {
            ...
            try
            {
                ...
                {
                    shapeName = Convert.ToString(((dynamic)hyperlink).Parent.Parent.Name);
                }
                else if (type == MsoHyperlinkType.msoHyperlinkRange)
                {
                    text = Convert.ToString(((dynamic)hyperlink).Parent.Parent.Text);
                    shapeName = Convert.ToString(((dynamic)hyperlink).Parent.Parent.Parent.Parent.Name);
                }
            }
            catch
            {
            ...
            return new PP.Hyperlink
            {
                ...
                // ShowAndReturn is not exposed by the Microsoft.Office.Interop Hyperlink wrapper; read it late-bound.
                ShowAndReturn = Int(() => ((dynamic)hyperlink).ShowAndReturn),
        ...
        // The parameter is the non-generic COM Shapes collection (Microsoft.Office.Interop's
        // Shapes does not implement IEnumerable<Shape>); the foreach casts each element to Shape.
        private static List<PP.Shape> BuildShapes(System.Collections.IEnumerable shapes, string outputFilePath, MsOfficeJsonWriterOptions options)
        {
            var list = new List<PP.Shape>();
        ...
        private static PP.Shape BuildShape(Shape shape, string outputFilePath, MsOfficeJsonWriterOptions options)
        {
            ...
            var entity = new PP.Shape
            {
                ...
                Decorative = Int(() => ((dynamic)shape).Decorative),
                // GraphicStyle, InkXML and the Has* companions below are newer Office
                // members the Microsoft.Office.Interop wrappers do not expose; read them late-bound.
                GraphicStyle = Int(() => ((dynamic)shape).GraphicStyle),
                HasChart = Int(() => shape.HasChart),
                HasInkXML = Int(() => ((dynamic)shape).HasInkXML),
                HasSectionZoom = Int(() => ((dynamic)shape).HasSectionZoom),
                ...
                InkXML = Str(() => ((dynamic)shape).InkXML),
                IsNarration = Int(() => ((dynamic)shape).IsNarration),
        ...
        private static PP.ActionSetting BuildActionSetting(ActionSetting actionSetting, int mouseActivation)
        {
            return new PP.ActionSetting
            {
                ...
                // ShowAndReturn is not exposed by the Microsoft.Office.Interop ActionSetting wrapper; read it late-bound.
                ShowAndReturn = Int(() => ((dynamic)actionSetting).ShowAndReturn),
        ...
        private static PP.PlaceholderFormat BuildPlaceholderFormat(PlaceholderFormat placeholderFormat)
        {
            return new PP.PlaceholderFormat
            {
                ...
                // Position is not exposed by the Microsoft.Office.Interop PlaceholderFormat wrapper; read it late-bound.
                Position = Long(() => ((dynamic)placeholderFormat).Position),
        ...
        private static PP.Font2 BuildFont2(Office.Font2 font2)
        {
            return new PP.Font2
            {
                ...
                // AutoRotateNumbers is not exposed by the Microsoft.Office.Interop Font2 wrapper; read it late-bound.
                AutoRotateNumbers = Int(() => ((dynamic)font2).AutoRotateNumbers),
        ...
        private static PP.ColorScheme BuildColorScheme(ColorScheme colorScheme)
        {
            ...
            {
                Accent1 = Str(() => colorScheme[PpColorSchemeIndex.ppAccent1].RGB),
                Accent2 = Str(() => colorScheme[PpColorSchemeIndex.ppAccent2].RGB),
                Accent3 = Str(() => colorScheme[PpColorSchemeIndex.ppAccent3].RGB),
                Background = Str(() => colorScheme[PpColorSchemeIndex.ppBackground].RGB),
                Fill = Str(() => colorScheme[PpColorSchemeIndex.ppFill].RGB),
                Foreground = Str(() => colorScheme[PpColorSchemeIndex.ppForeground].RGB),
                NotSchemeColor = Str(() => colorScheme[PpColorSchemeIndex.ppNotSchemeColor].RGB),
                SchemeColorMixed = Str(() => colorScheme[PpColorSchemeIndex.ppSchemeColorMixed].RGB),
                Shadow = Str(() => colorScheme[PpColorSchemeIndex.ppShadow].RGB),
                Title = Str(() => colorScheme[PpColorSchemeIndex.ppTitle].RGB),
            };
        }
