...
{
    public class MsPowerPoint_Interop__Repo : MsPowerPoint__Repo__Interface, MsOfficeRunningAppExport__Repo__Interface
    {
        ...
        public async Task ExportAsync(IList<MsOfficeCountry_Row> countries, string filePath, Action<string> log)
        {
            await Task.Run(() =>
            {
                log("Launching Microsoft PowerPoint via Interop...");
                ...
                try
                {
                    pptApp.AfterNewPresentation += prs =>
                        log($"  [Event] AfterNewPresentation  — {prs.Name}");
                    pptApp.PresentationBeforeSave += (Presentation prs, ref bool cancel) =>
                        log($"  [Event] PresentationBeforeSave — {prs.Name}");

                    // Create the presentation WITH a window (msoTrue). A windowless presentation
                    // (msoFalse) makes PowerPoint fail SaveAs/Close/Quit with COM errors such as
                    // "An error occurred while PowerPoint was saving the file." Word/Excel create
                    // windowed documents, which is why only PowerPoint hit this.
                ...
                {
                    if (!quit) { try { pptApp.Quit(); } catch { /* best effort */ } }
        ...
        public Task ExportToRunningAppAsync(object hostApplication, IList<MsOfficeCountry_Row> countries, Action<string> log)
        {
            if (hostApplication is not Application pptApp)
                throw new ArgumentException(
                    "Expected a Microsoft.Office.Interop.PowerPoint.Application instance.", nameof(hostApplication));
        ...
        public async Task WriteDocumentJsonAsync(string sourcePath, string outputJsonPath, Action<string> log)
        {
            await Task.Run(() =>
            {
                ...
                var pptApp = new Application { Visible = MsoTriState.msoTrue };
                try
                {
