...
{
    // Browser IndexedDB implementation of CountriesNowSpaceApiManager__Repo__Interface.
    // Uses IndexedDb_Repo (custom JS interop) to persist JSON lists.
    // Data survives page reloads and is scoped to the browser origin (not the tab session).
    // All operations are async; read methods return empty when the key has not been written yet.
    public class CountriesNowSpaceApiManager__IndexedDb__Repo
        : CountriesNowSpaceApiManager__Repo__Interface
    {
        private readonly IndexedDb_Repo__Interface _storage;
        ...
        public CountriesNowSpaceApiManager__IndexedDb__Repo(IndexedDb_Repo__Interface storage)
        {
            _storage = storage;
        }
