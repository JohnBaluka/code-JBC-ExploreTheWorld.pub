...
{
    [Inject]
    private WordSaveAsJson__Interop__Interface SaveAsJsonInterop { get; set; } = default!;
    ...
    private async Task BuildJsonAsync()
    {
        ...
        try
        {
            var result = await SaveAsJsonInterop.GetDocumentAsJsonAsync();
    ...
    {
        if (_json is null) return;
        var name = string.IsNullOrWhiteSpace(_fileName) ? "MsWord.json" : _fileName + ".json";
