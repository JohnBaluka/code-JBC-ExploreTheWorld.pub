...
public abstract class FlaUITestBase : IDisposable
{
    private static readonly string AppPath =
        Environment.GetEnvironmentVariable("ETW_WINFORMAPP_NETF_PATH")
        ?? FindAppExecutable("JBC.ExploreTheWorld.AL.WinFormApp._netF.exe");
    ...
    /// occluded window captures whatever covers it) and waits for the form to finish
    /// painting before anything is captured or asserted.
    /// </summary>
    ...
    private static string FindAppExecutable(string exeName)
    {
        ...
        var candidates = new[]
        {
            Path.Combine(repoRoot, "src", "AL.WinFormApp._netF", "bin", "Debug",   "net481", exeName),
            Path.Combine(repoRoot, "src", "AL.WinFormApp._netF", "bin", "Release", "net481", exeName)
        };
