...
internal static class UITestSettings
{
    ...
    // Set ETW_TEST_VIDEO=false to disable (FlaUI: animated GIF via frame capture)
    internal static bool VideoEnabled =>
        Environment.GetEnvironmentVariable("ETW_TEST_VIDEO") is not "false";

    // Set ETW_TEST_SETTLE_MS to override. The Office window must be foregrounded and
    // given time to repaint after attach/resize, or screenshots capture stale pixels.
    internal static int SettleDelayMs =>
        int.TryParse(Environment.GetEnvironmentVariable("ETW_TEST_SETTLE_MS"), out var ms) ? ms : 2000;
    ...
    internal static string GetTestFolder(ITestOutputHelper output)
    {
        ...
        var projectName  = assemblyName.StartsWith(jbcPrefix) ? assemblyName.Substring(jbcPrefix.Length) : assemblyName;
        ...
        var subPath         = displayName.StartsWith(subPrefix) ? displayName.Substring(subPrefix.Length) : displayName;
