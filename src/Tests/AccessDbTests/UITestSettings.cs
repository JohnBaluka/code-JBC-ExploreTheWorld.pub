...
internal static class UITestSettings
{
    ...
    // Set ETW_TEST_SETTLE_MS to override. Access repaints the document area
    // asynchronously after opening a database or a form, so screenshots taken
    // immediately are blank.
    internal static int SettleDelayMs =>
        int.TryParse(Environment.GetEnvironmentVariable("ETW_TEST_SETTLE_MS"), out var ms) ? ms : 3000;
