...
[ProgId("JBC.ExploreTheWorld.AL.MsOfficeExcelVstoAddIn.Addin")]
[Guid("C2B3D4E5-F6A7-4890-BCDE-F01234567890")]
[ComVisible(true)]
public class Addin : IDTExtensibility2, IRibbonExtensibility
{
    private static readonly string _addinOfficeRegistryKey = @"Software\Microsoft\Office\Excel\Addins\";
    private static readonly string _progId               = "JBC.ExploreTheWorld.AL.MsOfficeExcelVstoAddIn.Addin";
    private static readonly string _addinFriendlyName    = "Explore the World Excel Add-in";
    private static readonly string _addinDescription     = "Explore the World Excel Countries + Watcher";
    ...
    private MsExcel_Watcher_WebView_Form?    _watcherBlazorForm;
        ...
        try
        {
            ...
            var thread = new System.Threading.Thread(() =>
            {
                try
                {
                    ...
                    form.Text = "ETW MsOfficeExcelVstoAddIn";
    ...
    public void OnSaveAsJson(IRibbonControl control)
    {
        try
        {
            ...
            if (wb == null) { LogInfo("OnSaveAsJson", "no active workbook"); return; }
            var thread = new System.Threading.Thread(() =>
            {
                try
                {
    ...
    public void OnSaveAsJsonVba(IRibbonControl control)
    {
        ...
        {
            if (_application == null) return;
            _application.Run("MSO_MsExcel.WriteActiveWorkbook");
        }
        ...
        try
        {
            ...
            var thread = new System.Threading.Thread(() =>
            {
                try
                {
                    ...
                    LogInfo("OnOpenWatcherBlazorForm", "constructing MsExcel_Watcher_WebView_Form");
                    var form = new MsExcel_Watcher_WebView_Form(services);
                    form.Text = "ETW MsOfficeExcelVstoAddIn";
    ...
    private void WireExcelEvents()
    {
        ...
        try
        {
            var activeWb   = TrySafeGet(() => _application.ActiveWorkbook?.Name);
            var openFiles  = GetOpenWorkbookNames();
            _watcherSvc.Excel_SetConnected(true, activeWb);
            _watcherSvc.Excel_UpdateOpenFiles(openFiles, activeWb);
            _watcherSvc.Excel_AppendLog($"Connected to Excel {_application.Version}");

            _application.WorkbookOpenEvent         += OnExcel_WorkbookOpen;
            _application.NewWorkbookEvent          += OnExcel_NewWorkbook;
            _application.WorkbookBeforeCloseEvent  += OnExcel_WorkbookBeforeClose;
            _application.WorkbookBeforeSaveEvent   += OnExcel_WorkbookBeforeSave;
            _application.WorkbookActivateEvent     += OnExcel_WorkbookActivate;
            _application.WorkbookDeactivateEvent   += OnExcel_WorkbookDeactivate;
            _application.SheetSelectionChangeEvent += OnExcel_SheetSelectionChange;
        }
        catch (Exception ex) { LogError("WireExcelEvents", ex); }
    }
    ...
    private void OnExcel_WorkbookOpen(Excel.Workbook wb)
    {
        if (_watcherSvc == null) return;
        _watcherSvc.Excel_AppendLog($"WorkbookOpen: {TrySafeGet(() => wb.Name)}");
        _watcherSvc.Excel_UpdateOpenFiles(GetOpenWorkbookNames(), TrySafeGet(() => wb.Name));
    }

    private void OnExcel_NewWorkbook(Excel.Workbook wb)
    {
        if (_watcherSvc == null) return;
        _watcherSvc.Excel_AppendLog($"NewWorkbook: {TrySafeGet(() => wb.Name)}");
        _watcherSvc.Excel_UpdateOpenFiles(GetOpenWorkbookNames(), TrySafeGet(() => wb.Name));
    }

    private void OnExcel_WorkbookBeforeClose(Excel.Workbook wb, ref bool cancel)
        => _watcherSvc?.Excel_AppendLog($"WorkbookBeforeClose: {TrySafeGet(() => wb.Name)}");

    private void OnExcel_WorkbookBeforeSave(Excel.Workbook wb, bool saveAsUI, ref bool cancel)
        => _watcherSvc?.Excel_AppendLog($"WorkbookBeforeSave: {TrySafeGet(() => wb.Name)}");

    private void OnExcel_WorkbookActivate(Excel.Workbook wb)
    {
        if (_watcherSvc == null) return;
        var name = TrySafeGet(() => wb.Name);
        _watcherSvc.Excel_AppendLog($"WorkbookActivate: {name}");
        _watcherSvc.Excel_UpdateOpenFiles(GetOpenWorkbookNames(), name);
    }

    private void OnExcel_WorkbookDeactivate(Excel.Workbook wb)
        => _watcherSvc?.Excel_AppendLog($"WorkbookDeactivate: {TrySafeGet(() => wb.Name)}");

    private void OnExcel_SheetSelectionChange(NetOffice.ICOMObject sh, Excel.Range target)
        => _watcherSvc?.Excel_AppendLog("SheetSelectionChange");

    private List<string> GetOpenWorkbookNames()
    {
        var names = new List<string>();
        try
        {
            foreach (Excel.Workbook wb in _application!.Workbooks)
                names.Add(TrySafeGet(() => wb.Name) ?? "");
        }
        catch { }
        return names;
    }
    ...
    private static void RemoveLegacyTaskPaneProgIds()
    {
        ...
        [
            ...
            "JBC.ExploreTheWorld.AL.MsOfficeExcelVstoAddIn.CountriesNowSpace_UserControl",
            "JBC.ExploreTheWorld.AL.MsOfficeExcelVstoAddIn.MsExcel_Watcher_UserControl",
            "JBC.ExploreTheWorld.AL.MsOfficeExcelVstoAddIn.CountriesNowSpace_WebView_UserControl",
            "JBC.ExploreTheWorld.AL.MsOfficeExcelVstoAddIn.MsExcel_Watcher_WebView_UserControl",
        ];
    ...
    private static void LogError(string context, Exception ex)
    {
        try
        {
            string path = Path.Combine(Path.GetTempPath(), "JBC.ExploreTheWorld.AL.MsOfficeExcelVstoAddIn.log");
