...
[COMAddin("JBC ETW Word Watcher", "ExploreTheWorld Word event watcher", 3)]
[ProgId("JBC.ExploreTheWorld.AL.MsOfficeWordVstoAddIn._netF.Addin")]
[Guid("E4D5E6F7-B8C9-4A12-DEF0-123456789012")]
[CustomUI("RibbonUI.xml", true)]
// NOTE: no [CustomPane]. Word is SDI — every document has its own top-level window and a Custom
// Task Pane belongs to exactly one window. NetOffice's [CustomPane] creates a single pane bound to
// whichever window was active when the add-in loaded, so it only ever appears on that first
// document. Instead we create a pane per window on demand, keyed by window Hwnd — see the
// "Per-window Custom Task Panes" region below.
public class Addin : COMAddin
{
    // ------------------------------------------------------------------ //
    //  Per-window Custom Task Panes (Word SDI)
    // ------------------------------------------------------------------ //
    private const string WatcherPaneProgId   = "JBC.ExploreTheWorld.AL.WinFormsLib.MsWord_Watcher_UserControl";
    private const string CountriesPaneProgId = "JBC.ExploreTheWorld.AL.MsOfficeWordVstoAddIn.CountriesNow_TaskPane_UserControl";
    private const string WatcherPaneTitle    = "ETW MsOfficeWordVstoAddIn._netF — Watcher";
    private const string CountriesPaneTitle  = "ETW MsOfficeWordVstoAddIn._netF — Countries API";

    // One pane instance per document window, keyed by Window.Hwnd.
    ...
    // When this add-in is COM-activated into WINWORD.EXE, dependencies are resolved against the
    // host's default AppDomain — whose probe path is Word's install folder and the GAC, NOT this
    // add-in's bin directory. On top of that, the EF Core / Sqlite stack references older assembly
    // versions (e.g. Microsoft.Bcl.AsyncInterfaces 1.0.0.0) than the ones we deploy (10.0.0.9), and
    // a .dll.config binding redirect is not honored for a COM-activated library. This resolver loads
    ...
    static Addin()
    {
        ...
        try
        {
            ...
            NetOffice.DebugConsole.Default.FileName = Path.Combine(Path.GetTempPath(),
                "JBC.ExploreTheWorld.AL.MsOfficeWordVstoAddIn._netF.netoffice.log");
        }
        catch { }
    ...
    // There are no [CustomPane]s to create — panes are made per document window on demand.
    ...
    private static int SafeHwnd(Word.Window window)
    {
    ...
    // Get (or lazily create) the pane of the given kind for a specific document window.
    // Returns null if the CTP factory or window is unavailable, or CreateCTP fails.
    private Office.CustomTaskPane GetOrCreatePane(
        Dictionary<int, Office.CustomTaskPane> map, string progId, string title, int width, Word.Window window)
    {
        int hwnd = SafeHwnd(window);
        if (map.TryGetValue(hwnd, out var existing) && existing != null)
        {
            try { _ = existing.Visible; return existing; }   // still alive on this window?
            catch { map.Remove(hwnd); }                        // pane died with a closed window — recreate
        }
        ...
        try
        {
            ...
            // The Watcher control has no DI ctor — inject the running application so it goes live.
            if (progId == WatcherPaneProgId)
            {
                try { if (pane.ContentControl is MsWord_Watcher_UserControl uc) uc.InjectApplication(Application); }
    ...
    // A pane's visibility changed (toggle or the pane's own [X]) — refresh both toggle buttons.
    private void OnPaneVisibleStateChanged(Office._CustomTaskPane customTaskPaneInst)
    {
        try
        {
            if (RibbonUI == null) return;
            try { RibbonUI.InvalidateControl("etw_WordTaskPane"); } catch { }
    ...
    public void OnOpenWatcher(Office.IRibbonControl control)
    {
        try
        {
            ...
            {
                Text          = "Microsoft Word - Watcher",
                Size          = new System.Drawing.Size(940, 660),
                MinimumSize   = new System.Drawing.Size(700, 480),
                StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
            };
            var uc = new MsWord_Watcher_UserControl { Dock = System.Windows.Forms.DockStyle.Fill };
        ...
        try
        {
            var thread = new System.Threading.Thread(() =>
            {
                try
                {
                    ...
                    form.EnableAddinExportMode("Word");
    ...
    // Must not throw — an exception out of a getPressed callback leaves the ribbon toggle stuck.
    ...
    {
        try { Application?.Run("MSO_MsWord.WriteActiveDocument"); }
    ...
    private static void LogError(string context, Exception ex)
    {
        try
        {
            string path = Path.Combine(Path.GetTempPath(), "JBC.ExploreTheWorld.AL.MsOfficeWordVstoAddIn._netF.log");
