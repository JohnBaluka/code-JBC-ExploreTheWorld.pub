...
{
    [Inject]
    private PowerPointEvents__Interop__Interface EventsInterop { get; set; } = default!;
    ...
    // "selectionChanged" uses the Office common API (DocumentSelectionChanged) and works on
    // all hosts; "slideSelectionChanged" needs the preview-only PowerPointApi BETA event and
    // is skipped with a console warning on production hosts.
    private readonly List<EventDescriptor> _events =
    [
        new("selectionChanged",      "Selection Changed"),
        new("slideSelectionChanged", "Slide Selection Changed (preview API)", enabled: false),
    ];
