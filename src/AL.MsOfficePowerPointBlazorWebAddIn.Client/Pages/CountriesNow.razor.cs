...
public partial class CountriesNow : ComponentBase
{
    ...
    [Inject]
    private PowerPointCountriesExport__Interop__Interface ExportInterop { get; set; } = default!;
    ...
    private async Task ExportToDocumentAsync()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(result.Error))
                ...
                _exportStatus = $"Added {result.Count} slides ({result.FlagCount} flags) to the presentation."
                    + (string.IsNullOrEmpty(result.FlagError) ? "" : $" Flag insertion issue: {result.FlagError}");
        }
