...
{
    [Inject]
    private PowerPointSaveAsJson__Interop__Interface SaveAsJsonInterop { get; set; } = default!;
    ...
    private async Task BuildJsonAsync()
    {
        ...
        try
        {
            var result = await SaveAsJsonInterop.GetPresentationAsJsonAsync();
    ...
    {
        if (_json is null) return;
        var name = string.IsNullOrWhiteSpace(_fileName) ? "MsPowerPoint.json" : _fileName + ".json";
